...
<!--{assign var=default_layout value='<span style="font-size:10px">タイムセール終了まで</span><br /><span style="font-bold:bold"><b>{dn}{dl}と{hnn}時間{mnn}分{snn}秒</b> {desc}</span>'}-->
...
<!--{include file="`$smarty.const.S_TEMPLATE_PATH`/timesales/countdown_layout.tpl" assign=layout}-->
<!--{include file="`$smarty.const.S_TEMPLATE_PATH`/timesales/countdown_layout2.tpl" assign=layout2}-->
<!--{include file="`$smarty.const.S_TEMPLATE_PATH`/timesales/msg_before_start.tpl" assign=msg_before_start}-->
<!--{include file="`$smarty.const.S_TEMPLATE_PATH`/timesales/msg_after_stoped.tpl" assign=msg_after_stoped}-->
	    ...
	    <!--{foreach from=$arrProductsClass item=product key=idx}-->
	    	<!--{if $product.timesales_flg == 1 && $product.timesales_term_type == 2}-->
	    		...
	    		objTimeSaleses[<!--{$idx}-->].addOnExpiry(function() {
	    				...
	    				<!--{if $smarty.server.SCRIPT_NAME == '/smp/cart/index.php'}-->
	    				$('input[name="cart_no"]').val('<!--{$product.cart_no}-->');
	    				$('input[name="mode"]').val('delete');
	    				$('#form1').submit();
	    				<!--{elseif $smarty.server.SCRIPT_NAME == '/smp/shopping/confirm.php'}-->
