...
<form name="member_form" id="member_form" method="post" action="<!--{$smarty.const.MOBILE_URL_DIR}-->entry/kiyaku.php">
	<input type="hidden" name="PHPSESSID" value="<!--{$smarty.request.PHPSESSID}-->">
...
<!--▼会員登録がお済みのお客様-->
<form name="member_form" id="member_form" method="post" action="<!--{$smarty.const.MOBILE_SSL_URL}-->shopping/deliv.php">
	<input type="hidden" name="PHPSESSID" value="<!--{$smarty.request.PHPSESSID}-->">
	<input type="hidden" name="transactionid" value="<!--{$smarty.session.transactionid}-->">
...
<a href="<!--{$smarty.const.MOBILE_URL_CART_TOP}-->?transactionid=<!--{$smarty.session.transactionid}-->&PHPSESSID=<!--{$smarty.request.PHPSESSID}-->" accesskey="9"><!--{9|numeric_emoji}-->かごを見る</a><br>
