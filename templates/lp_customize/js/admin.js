...
// 送料無料項目判定
function fnCheckDelivFree(icolor) {
	list = new Array(
		'deliv_free_num'
		);
	if(document.form1['deliv_free_id'].value == 4 || document.form1['deliv_free_id'].value == 5) {
		fnChangeDisabled(list, '');
		if (!document.form1['deliv_free_num'].value){
			document.form1['deliv_free_num'].value = "0";
		}
	} else {
		fnChangeDisabled(list, icolor);
		document.form1['deliv_free_num'].value = "";
	}
}
...
// セット商品判定
function fnCheckSetLimit(icolor) {
	if(document.form1['set_item']) {
		list = new Array(
// 			'category_id',
// 			'price01',
// 			'price02',
...
// 			'sale_limit'
		...
		if(document.form1['set_item'].checked) {
			fnChangeDisabled(list, icolor);
		} else {
			fnChangeDisabled(list, '');
		}
...
// 		fm['product_code:' + i].value = fm['product_code:1'].value;
		...
		fm['cost_price:' + i].value = fm['cost_price:1'].value;
	}
}
