...
<div id="mypagecolumn">
  ...
  <div id="mycontentsarea">
    <form name="form1" method="post" action="./gmopg_change_card_info.php">
    <input type="hidden" name="mode" value="" />
    <input type="hidden" name="payment_wallet_id" value="" />
    ...
    <!--<h3 class="mycont_headttl">▼ 登録済みクレジットカード</h3>-->
            <div class="card_tb">
                  <!--{if !$noSavedCard}-->
                  <!--{foreach from=$arrSavedCardInfo item=card_info key=payment_wallet_id name=card_info}-->
                <table summary="カード情報照会" class="mgb15">
                  <tr>
                    <th colspan="2">
                        <div class="card_ttl">
                            <h3>クレジットカード情報<!--{$smarty.foreach.card_info.iteration|escape}--></h3>
                            <span class="fl_r cardch_bt pst4"><a href="#" class="edit-card-info" data-payment-wallet-id="<!--{$payment_wallet_id|escape}-->"><span class="pst5">カード情報を変更する</span></a></span>
                        </div>
                    </th>
                  </tr>
                  <tr>
            ...
            <div class="cardnew_bt"><a href="./gmopg_change_card_info.php">カード情報の新規登録はこちらから</a></div>
            <!--{/if}-->
        </div> 
    </form>
  </div>
...
$(function(){
    ...
    $('.edit-card-info').on('click',function(){
        $('input[name="payment_wallet_id"]').val($(this).data('paymentWalletId'));
        $('input[name="mode"]').val('editCardInfo');
        $('form[name="form1"]').attr('action' , './gmopg_change_card_info.php').submit();
    });
});
</script>
